...
TEST_CASE("r7.hpp (constrained)")
{
    ...
    {
        CHECK_U(saturation_pressure (300._K), 0.353658941e-2_MPa, 1e-8);
        CHECK_U(saturation_pressure (500._K), 0.263889776e1_MPa, 1e-8);
        CHECK_U(saturation_pressure (600._K), 0.123443146e2_MPa, 1e-8);
        CHECK_U(saturation_temperature (0.1_MPa), 0.372755919e3_K, 1e-8);
        CHECK_U(saturation_temperature (1._MPa ), 0.453035632e3_K, 1e-8);
        CHECK_U(saturation_temperature (10._MPa), 0.584149488e3_K, 1e-8);
    }
    SUBCASE("regions")
    {
            using namespace isto::iapws::r7::detail;
        CHECK_U(b23 (0.62315e3_K), 0.165291643e2_MPa, 1e-8);
        CHECK_U(b23i (0.165291643e2_MPa), 0.62315e3_K, 1e-8);
